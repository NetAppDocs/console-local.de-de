---
sidebar: sidebar 
permalink: reference-local-known-limitations.html 
keywords: console local deployment, known limitations, unsupported platforms, unsupported devices, unsupported functions, IP range 
summary: Referenz für bekannte Einschränkungen bei der lokalen Bereitstellung der NetApp Console. 
---
= Bekannte Einschränkungen bei der lokalen Bereitstellung der NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
:linkattrs: 
:imagesdir: ./media/


[role="lead"]
Bekannte Einschränkungen kennzeichnen Plattformen, Geräte oder Funktionen, die von dieser Produktversion nicht unterstützt werden oder nicht korrekt mit ihr zusammenarbeiten. Überprüfen Sie diese Einschränkungen sorgfältig.



[[console-vm-limitations]]
== Einschränkungen der Console-VM



[[possible-conflict-with-ip-addresses-in-the-10-42-0-016-and-10-43-0-016-ranges]]
=== Möglicher Konflikt mit IP-Adressen in den Bereichen 10.42.0.0/16 und 10.43.0.0/16

NetApp Console lokale Bereitstellung verwendet einen festen Adressraum für die Kommunikation zwischen den Diensten. Die IP-Bereiche 10.42.0.0/16 und 10.43.0.0/16 werden für das interne Netzwerk verwendet. Vor der Bereitstellung der Console lokalen Bereitstellung ist sicherzustellen, dass diese IP-Bereiche nicht anderen VM-Instanzen, DHCP-Bereichen, DNS-Einträgen oder Load Balancer VIP-Pools in den für die Console lokale Bereitstellung verfügbaren VLANs zugewiesen sind.

Im Knowledge Base Artikel „Agent IP conflict with existing network“ finden sich Anweisungen zum Ändern der IP-Adresse der Schnittstellen des Agenten.



[[shared-linux-hosts-are-not-supported]]
=== Shared Linux Hosts werden nicht unterstützt

NetApp Console lokale Bereitstellung wird auf einer VM, die mit anderen Anwendungen geteilt wird, nicht unterstützt. Die VM muss ausschließlich für die NetApp Console lokale Bereitstellung vorgesehen sein.



[[third-party-agents-and-extensions]]
=== Drittanbieter-Agenten und Erweiterungen

Drittanbieter-Agenten oder VM-Erweiterungen werden auf der lokalen NetApp Console-Bereitstellungs-VM nicht unterstützt.



[[identity-and-access-limitations]]
== Identitäts- und Zugriffsbeschränkungen

[cols="1,3"]
|===
| Bereich | Einschränkung 


| Lokale Benutzerregistrierung und Kontowiederherstellung | Ein SMTP-E-Mail-Server muss konfiguriert sein, bevor lokale Benutzer eingeladen werden können oder die Passwortzurücksetzung für lokale Benutzer genutzt werden kann. 


| Active Directory Integration | Doppelte E-Mail-Adressen werden für lokale Benutzer und Active Directory Benutzer nicht unterstützt. Wenn sich beispielsweise der ursprüngliche lokale Organisationsadministrator später selbst als Active Directory Benutzer hinzufügt, muss eine andere E-Mail-Adresse verwendet werden. 
|===